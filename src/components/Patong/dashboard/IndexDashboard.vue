...
export default {
  ...
  mounted() {
    ...
    this.$axios.get("https://sentimentanalysis.chochiang.com/tourist/beach/Auto-sentiment-web/API/patong/counts/all.json").then(({ data }) => {
      for (const key in data) {
        this.countComment = data[key].numComment;
      }
   
    this.$axios.get("https://sentimentanalysis.chochiang.com/tourist/beach/Auto-sentiment-web/API/patong/counts/pos.json").then(({ data }) => {
      console.log(data);
      for (const key in data) {
    ...
    this.$axios.get("https://sentimentanalysis.chochiang.com/tourist/beach/Auto-sentiment-web/API/patong/counts/neg.json").then(({ data }) => {
      console.log(data);
      for (const key in data) {
